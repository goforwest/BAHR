...
BAHR_TO_TAFAIL = {
    ...
    2: {"primary": "مُتَفَاعِلُنْ مُتَفَاعِلُنْ مُتَفَاعِلُنْ", "variations": []},  # الكامل (al-Kamil)
    3: {"primary": "مُفَاعَلَتُنْ مُفَاعَلَتُنْ فَعُولُنْ", "variations": []},  # الوافر (al-Wafir)
    4: {  # الرمل (ar-Ramal)
    ...
    7: {"primary": "مُسْتَفْعِلُنْ مُسْتَفْعِلُنْ مُسْتَفْعِلُنْ", "variations": []},  # الرجز (ar-Rajaz)
    8: {  # الهزج (al-Hazaj)
...
def pattern_to_tafail(pattern: str) -> List[str]:
    ...
    sorted_patterns = sorted(
        BASIC_TAFAIL.items(), key=lambda x: len(x[0]), reverse=True
    )
...
def perform_taqti3(
    verse: str, normalize: bool = True, bahr_id: Optional[int] = None
) -> str:
    """
    ...
    if bahr_id is not None:
        if not isinstance(bahr_id, int):
            raise ValueError(
                f"bahr_id must be an integer, got {type(bahr_id).__name__}"
            )
    ...
    if not tafail:
        raise ValueError(
            "Could not identify any tafa'il in verse - text may not be poetry"
        )
