...
class BahrException(Exception):
    def __init__(self, code: str, message: str, *, severity: str = "error", can_retry: bool = False, context: Optional[Dict[str, Any]] = None):
...
class InputError(BahrException):
    def __init__(self, code: str = "ERR_INPUT_001", message: str = "Invalid input", **kw: Any):
        super().__init__(code, message, severity="error", can_retry=False, **kw)


class AnalysisError(BahrException):
    def __init__(self, code: str = "ERR_ANALYSIS_001", message: str = "Analysis failed", **kw: Any):
        super().__init__(code, message, severity="error", **kw)


class RateLimitError(BahrException):
    def __init__(self, code: str = "ERR_RATE_001", message: str = "Rate limit exceeded", **kw: Any):
        super().__init__(code, message, severity="warning", can_retry=True, **kw)


class AuthError(BahrException):
    def __init__(self, code: str = "ERR_AUTH_001", message: str = "Unauthorized", **kw: Any):
        super().__init__(code, message, severity="error", can_retry=False, **kw)


class UnknownError(BahrException):
    def __init__(self, code: str = "ERR_UNKNOWN_001", message: str = "Unexpected error", **kw: Any):
