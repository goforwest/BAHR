...
def analyze_distribution(verses):
    ...
    # Count verses with meter labels
    verses_with_meters = [v for v in verses if v.get('meter')]

    # Count by meter name
    meter_counts = Counter(v.get('meter') for v in verses_with_meters)
    ...
    total = len(verses_with_meters)
    print(f"Total verses with meter labels: {total}")
    print(f"Unique meters: {len(meter_counts)}")
    print()

    print("Meter Distribution:")
    print("-" * 80)
    print(f"{'Rank':<6} {'Meter':<30} {'Count':<8} {'%':<8} {'Priority':<10}")
    print("-" * 80)
    ...
    for rank, (meter, count) in enumerate(sorted_meters, 1):
        ...
        print(f"{rank:<6} {meter:<30} {count:<8} {percentage:>6.1f}%  {priority:<10}")
    ...
    print(f"Expected accuracy gain: +5-8 pp (66% → 71-74%)")
    print("=" * 80)
    print()
...
def main():
    ...
    print(f"Loaded {len(verses)} total verses from golden dataset")
    print()
